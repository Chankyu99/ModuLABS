...
from vision.ocr_pipeline import VisionPipeline, DetectionResult
...
def process_vision_result(results: list[DetectionResult], current_slots: dict) -> tuple[dict, Optional[str]]:
    """
    비전 파이프라인 결과(DetectionResult)를 챗봇 슬롯에 매핑.
    반환: (updated_slots, vision_message)
    """
    if not results:
        return current_slots, None

    # 가장 신뢰도가 높거나 첫 번째 객체 기준 (단순화)
    det = results[0]
    updated_slots = {**current_slots}
    
    # 1. 물품 클래스 매핑
    updated_slots["item"] = det.class_name
    
    # 2. 상세 수치 매핑
    vision_msg = f"📸 이미지 분석 결과, **{det.class_name}**으로 확인되었습니다.\n"
    
    if det.specs:
        specs = det.specs
        if det.class_name == "power_bank":
            cap = specs.get("capacity_mah") or specs.get("capacity_wh")
            unit = "mAh" if specs.get("capacity_mah") else "Wh"
            if cap:
                updated_slots["quantity"] = f"{cap}{unit}"
                vision_msg += f"- 추출된 용량: {cap}{unit}\n"
        elif det.class_name == "liquid_cosmetics":
            vol = specs.get("volume_ml") or specs.get("weight_g")
            unit = "ml" if specs.get("volume_ml") else "g"
            if vol:
                updated_slots["quantity"] = f"{vol}{unit}"
                vision_msg += f"- 추출된 용량: {vol}{unit}\n"

    # 3. 신뢰도/규정 기반 피드백
    if det.requires_manual_check:
        vision_msg += "\n⚠️ **주의**: 글씨가 인식 오류로 추정치를 직접 알려주세요. (mAh, Wh, ml 등)"
    elif det.regulation:
        vision_msg += f"\n⚖️ **규정 판단**: {det.regulation.get('verdict', '확인 필요')}\n"
        vision_msg += f"({det.regulation.get('message', '')})"

    return updated_slots, vision_msg

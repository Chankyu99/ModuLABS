...
> 책 5장 내용 + [5.3~5.6 선형 모델 및 규제](https://github.com/Chankyu99/ModuLABS/blob/master/04_MachineLearning/Study/Regression/5.3_Gradient_Descent_5.4_LinearModel_5.5_Polynomial_5.6_Regularized_model.ipynb) + [5.7~5.8 로지스틱 회귀 및 회귀 트리](https://github.com/Chankyu99/ModuLABS/blob/master/04_MachineLearning/Study/Regression/5.7_로지스틱_회귀_5.8_회귀_트리.ipynb)
...
> 라쏘나 엘라스틱넷 모델과 다르게 릿지 회귀는 `alpha` 증가에 따른 성능 저하가 거의 나타나지 않고, 일반적인 선형 회귀 수준(0.746)을 비교적 안정적으로 유지하였다. 캘리포니아 주택 데이터셋 특성상 L2 규제를 가하더라도 회귀 모델 성능 폭 변화가 그리 크지 않음을 알 수 있다.
...
> 보스턴 주택 데이터와는 달리 캘리포니아 주택 데이터에서는 `alpha`가 커질수록 평균 RMSE가 눈에 띄게 증가하며 예측 성능이 떨어지는 양상을 띠었다. 이는 강력한 L1 규제로 인해 유의미한 피처들의 회귀 계수마저 0으로 과도하게 축소되면서 오히려 모델의 표현력을 저하시키는 원인이 되었기 때문이다.
...
> 라쏘 수행 결과보다는 L2 규제가 결합되어 에러 증가 폭이 조금 완화되긴 했으나, 여전히 `alpha`를 키울수록 오차가 커진다. 본 데이터 세트에서는 무리하게 규제를 가하는 것이 오히려 예측 성능을 떨어뜨리는 독이 됨을 알 수 있다.

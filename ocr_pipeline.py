...
# ── 프로젝트 모듈 ─────────────────────────────────────────────────
from spec_parser import SpecParser, RegulationChecker, ParsedSpecs
from risk_model_db import RiskModelDB
...
class VisionPipeline:
    ...
    def _process_ocr_class(self, det: DetectionResult, img_bgr) -> DetectionResult:
        ...
        # 수치 추출
        specs = self.parser.extract(det.ocr_text)
...
def test_all_custom_labels(pipeline: VisionPipeline):
    ...
    for folder_name, class_name in class_map.items():
        ...
        for img_path in images:
            ...
            # 수치를 하나라도 추출했으면 성공
            has_spec = det.specs and any(v is not None for v in det.specs.values())
            if has_spec:
                success += 1
